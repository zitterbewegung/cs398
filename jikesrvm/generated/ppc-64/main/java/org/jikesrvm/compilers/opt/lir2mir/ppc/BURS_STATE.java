...
/*static final byte arity[] = {
	...
	2,	// 156=BOOLEAN_CMP_ADDR
	-1,	// 157=BOOLEAN_CMP_LONG
	...
	1,	// 220=DCBZL
	1,	// 221=ICBI
	-1,	// 222=CALL_SAVE_VOLATILE
	-1,	// 223=MIR_START
	-1,	// 224=MIR_LOWTABLESWITCH
	-1,	// 225=PPC_DATA_INT
	-1,	// 226=PPC_DATA_LABEL
	-1,	// 227=PPC_ADD
	-1,	// 228=PPC_ADDr
	-1,	// 229=PPC_ADDC
	-1,	// 230=PPC_ADDE
...
static void closure_rp(BURS_TreeNode p, int c) {
	if (c < p.cost_rz) {
		p.cost_rz = (char)(c);
		p.word0 = (p.word0 & 0xFF0FFFFF) | 0x100000; // p.rz = 1
		closure_rz(p, c);
	}
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x8000; // p.rs = 1
		closure_rs(p, c);
	}
}
...
private void label_REF_MOVE(BURS_TreeNode p) {
	...
	if (	// rs: REF_MOVE(INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x30000; // p.rs = 6
			closure_rs(p, c);
		}
	}
	if (	// rs: REF_MOVE(INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x38000; // p.rs = 7
			closure_rs(p, c);
		}
	}
	if (	// rs: REF_MOVE(INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x40000; // p.rs = 8
			closure_rs(p, c);
		}
	}
...
private void label_LONG_DIV(BURS_TreeNode p) {
	p.word0 = 0;
	p.initCost();
	BURS_TreeNode lchild, rchild;
	lchild = p.child1;
...
private void label_LONG_REM(BURS_TreeNode p) {
	p.word0 = 0;
	p.initCost();
	BURS_TreeNode lchild, rchild;
	lchild = p.child1;
...
private void label_INT_SHL(BURS_TreeNode p) {
	...
	if (	// rz: INT_SHL(r,INT_CONSTANT)
		...
		if (c < p.cost_rz) {
			p.cost_rz = (char)(c);
			p.word0 = (p.word0 & 0xFF0FFFFF) | 0x200000; // p.rz = 2
			closure_rz(p, c);
		}
	}
	// rz: INT_SHL(r,r)
	...
	if (c < p.cost_rz) {
		p.cost_rz = (char)(c);
		p.word0 = (p.word0 & 0xFF0FFFFF) | 0x300000; // p.rz = 3
		closure_rz(p, c);
	}
	if (	// rz: INT_SHL(INT_USHR(r,INT_CONSTANT),INT_CONSTANT)
		...
		if (c < p.cost_rz) {
			p.cost_rz = (char)(c);
			p.word0 = (p.word0 & 0xFF0FFFFF) | 0x400000; // p.rz = 4
			closure_rz(p, c);
		}
	}
}
...
private void label_LONG_SHL(BURS_TreeNode p) {
	...
	if (	// r: LONG_SHL(LONG_USHR(r,INT_CONSTANT),INT_CONSTANT)
		...
		c = STATE(lchild.child1).cost_r + ((VLR(p) <= VR(p)) ? 10 : INFINITE);
		if (c < p.cost_r) {
			p.cost_r = (char)(c);
...
private void label_INT_SHR(BURS_TreeNode p) {
	...
	if (	// rs: INT_SHR(r,INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x10000; // p.rs = 2
			closure_rs(p, c);
		}
	}
	// rs: INT_SHR(r,r)
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x18000; // p.rs = 3
		closure_rs(p, c);
	}
...
private void label_INT_USHR(BURS_TreeNode p) {
	...
	if (c < p.cost_rz) {
		p.cost_rz = (char)(c);
		p.word0 = (p.word0 & 0xFF0FFFFF) | 0x500000; // p.rz = 5
		closure_rz(p, c);
	}
...
private void label_REF_AND(BURS_TreeNode p) {
	...
	if (	// czr: REF_AND(r,INT_CONSTANT)
		...
		if (c < p.cost_czr) {
			p.cost_czr = (char)(c);
			p.word0 = (p.word0 & 0xFFFFBFFF) | 0x4000; // p.czr = 1
			closure_czr(p, c);
		}
	}
...
private void label_INT_2ADDRZerExt(BURS_TreeNode p) {
	p.word0 = 0;
	p.initCost();
	BURS_TreeNode lchild;
	lchild = p.child1;
	...
	c = STATE(lchild).cost_rz + 10;
	if (c < p.cost_rz) {
		p.cost_rz = (char)(c);
		p.word0 = (p.word0 & 0xFF0FFFFF) | 0x600000; // p.rz = 6
		closure_rz(p, c);
	}
	...
	if (c < p.cost_rz) {
		p.cost_rz = (char)(c);
		p.word0 = (p.word0 & 0xFF0FFFFF) | 0x700000; // p.rz = 7
		closure_rz(p, c);
	}
	if (	// rz: INT_2ADDRZerExt(INT_LOAD(r,INT_CONSTANT))
		...
		c = STATE(lchild.child1).cost_r + 11;
		if (c < p.cost_rz) {
			p.cost_rz = (char)(c);
			p.word0 = (p.word0 & 0xFF0FFFFF) | 0x800000; // p.rz = 8
			closure_rz(p, c);
		}
	}
	if (	// rz: INT_2ADDRZerExt(INT_LOAD(r,r))
		...
		c = STATE(lchild.child1).cost_r + STATE(lchild.child2).cost_r + 11;
		if (c < p.cost_rz) {
			p.cost_rz = (char)(c);
			p.word0 = (p.word0 & 0xFF0FFFFF) | 0x900000; // p.rz = 9
			closure_rz(p, c);
		}
	}
	if (	// rz: INT_2ADDRZerExt(INT_LOAD(REF_ADD(r,INT_CONSTANT),INT_CONSTANT))
		lchild.getOpcode() == INT_LOAD_opcode && 
		lchild.child1.getOpcode() == REF_ADD_opcode && 
		lchild.child1.child2.getOpcode() == INT_CONSTANT_opcode && 
		lchild.child2.getOpcode() == INT_CONSTANT_opcode 
	) {
		c = STATE(lchild.child1.child1).cost_r + (SI16(VLR(p)+VLLR(p)) ? 14 : INFINITE);
		if (c < p.cost_rz) {
			p.cost_rz = (char)(c);
			p.word0 = (p.word0 & 0xFF0FFFFF) | 0xA00000; // p.rz = 10
			closure_rz(p, c);
		}
	}
}
...
private void label_INT_2LONG(BURS_TreeNode p) {
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x90000; // p.rs = 18
		closure_rs(p, c);
	}
	// rs: INT_2LONG(rs)
	c = STATE(lchild).cost_rs + 10;
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x98000; // p.rs = 19
		closure_rs(p, c);
	}
}
...
private void label_INT_2BYTE(BURS_TreeNode p) {
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x20000; // p.rs = 4
		closure_rs(p, c);
	}
}
...
private void label_INT_2SHORT(BURS_TreeNode p) {
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x28000; // p.rs = 5
		closure_rs(p, c);
	}
}
...
private void label_BYTE_LOAD(BURS_TreeNode p) {
	...
	if (	// rs: BYTE_LOAD(r,INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x48000; // p.rs = 9
			closure_rs(p, c);
		}
	}
	// rs: BYTE_LOAD(r,r)
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x50000; // p.rs = 10
		closure_rs(p, c);
	}
}
...
private void label_SHORT_LOAD(BURS_TreeNode p) {
	...
	if (	// rs: SHORT_LOAD(r,INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x58000; // p.rs = 11
			closure_rs(p, c);
		}
	}
	// rs: SHORT_LOAD(r,r)
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x60000; // p.rs = 12
		closure_rs(p, c);
	}
}
...
private void label_INT_LOAD(BURS_TreeNode p) {
	...
	if (	// rs: INT_LOAD(r,INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x68000; // p.rs = 13
			closure_rs(p, c);
		}
	}
	if (	// rs: INT_LOAD(r,REF_MOVE(ADDRESS_CONSTANT))
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x70000; // p.rs = 14
			closure_rs(p, c);
		}
	}
	// rs: INT_LOAD(r,r)
	...
	if (c < p.cost_rs) {
		p.cost_rs = (char)(c);
		p.word0 = (p.word0 & 0xFFF07FFF) | 0x78000; // p.rs = 15
		closure_rs(p, c);
	}
	if (	// rs: INT_LOAD(REF_ADD(r,r),INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x80000; // p.rs = 16
			closure_rs(p, c);
		}
	}
	if (	// rs: INT_LOAD(REF_ADD(r,INT_CONSTANT),INT_CONSTANT)
		...
		if (c < p.cost_rs) {
			p.cost_rs = (char)(c);
			p.word0 = (p.word0 & 0xFFF07FFF) | 0x88000; // p.rs = 17
			closure_rs(p, c);
		}
	}
}
...
public void label(BURS_TreeNode p) {
	...
	case LONG_DIV_opcode: label_LONG_DIV(p); break;
	...
	case LONG_REM_opcode: label_LONG_REM(p); break;
	...
	case INT_2ADDRZerExt_opcode: label_INT_2ADDRZerExt(p); break;
...
static BURS_TreeNode kids(BURS_TreeNode p, int eruleno, int kidnumber)  { 
	...
	case 1: // stm: r
		...
		break;
	case 240: // r: REF_MOVE(LONG_CONSTANT)
	case 239: // r: REF_MOVE(ADDRESS_CONSTANT)
	case 217: // stm: IR_PROLOGUE
	case 210: // r: GET_TIME_BASE
	case 205: // stm: RETURN(NULL)
	case 204: // stm: GOTO
	case 137: // rs: REF_MOVE(INT_CONSTANT)
	case 136: // rs: REF_MOVE(INT_CONSTANT)
	case 135: // rs: REF_MOVE(INT_CONSTANT)
	case 37: // stm: TRAP
	case 29: // stm: READ_CEILING
	case 28: // stm: WRITE_FLOOR
	...
	case 2: // r: REGISTER
		break;
	case 262: // r: ATTEMPT_ADDR(r,r)
	case 261: // r: PREPARE_ADDR(r,r)
	case 254: // r: LONG_LOAD(r,r)
	case 242: // stm: LONG_IFCMP(r,r)
	case 241: // r: LONG_CMP(r,r)
	case 231: // r: LONG_USHR(r,r)
	case 229: // r: LONG_SHR(r,r)
	case 225: // r: LONG_SHL(r,r)
	case 222: // r: LONG_REM(r,r)
	case 220: // r: LONG_DIV(r,r)
	case 219: // r: LONG_MUL(r,r)
	case 216: // r: ATTEMPT_LONG(r,r)
	case 215: // r: ATTEMPT_INT(r,r)
	case 214: // r: PREPARE_LONG(r,r)
	case 213: // r: PREPARE_INT(r,r)
	case 212: // r: YIELDPOINT_OSR(any,any)
	case 211: // r: OTHER_OPERAND(r,r)
	case 209: // r: SYSCALL(r,any)
	case 207: // r: CALL(r,any)
	case 203: // stm: DOUBLE_CMPG(r,r)
	case 202: // stm: DOUBLE_CMPL(r,r)
	case 201: // stm: FLOAT_CMPG(r,r)
	case 200: // stm: FLOAT_CMPL(r,r)
	case 199: // stm: DOUBLE_IFCMP(r,r)
	case 198: // stm: FLOAT_IFCMP(r,r)
	case 196: // stm: INT_IFCMP2(r,r)
	case 181: // stm: INT_IFCMP(r,r)
	case 158: // rs: INT_LOAD(r,r)
	case 155: // r: DOUBLE_LOAD(r,r)
	case 152: // r: FLOAT_LOAD(r,r)
	case 149: // rp: USHORT_LOAD(r,r)
	case 147: // rs: SHORT_LOAD(r,r)
	case 145: // rp: UBYTE_LOAD(r,r)
	case 141: // rs: BYTE_LOAD(r,r)
	case 106: // r: DOUBLE_DIV(r,r)
	case 105: // r: FLOAT_DIV(r,r)
	case 104: // r: DOUBLE_SUB(r,r)
	case 103: // r: FLOAT_SUB(r,r)
	case 102: // r: DOUBLE_MUL(r,r)
	case 101: // r: FLOAT_MUL(r,r)
	case 100: // r: DOUBLE_ADD(r,r)
	case 99: // r: FLOAT_ADD(r,r)
	case 93: // r: REF_XOR(r,r)
	case 89: // r: REF_OR(r,r)
	case 82: // r: REF_AND(r,r)
	case 78: // rz: INT_USHR(r,r)
	case 75: // rs: INT_SHR(r,r)
	case 72: // rz: INT_SHL(r,r)
	case 68: // r: INT_REM(r,r)
	case 66: // r: INT_DIV(r,r)
	case 65: // r: INT_MUL(r,r)
	case 62: // r: REF_SUB(r,r)
	case 59: // r: REF_ADD(r,r)
	case 49: // boolcmp: BOOLEAN_CMP_ADDR(r,r)
	case 47: // r: BOOLEAN_CMP_ADDR(r,r)
	case 45: // boolcmp: BOOLEAN_CMP_INT(r,r)
	case 43: // r: BOOLEAN_CMP_INT(r,r)
	case 38: // stm: TRAP_IF(r,r)
	case 13: // any: OTHER_OPERAND(any,any)
		if (kidnumber == 0)  return p.child1;
		if (kidnumber == 1)  return p.child2;
		break;
	case 253: // r: LONG_LOAD(r,REF_MOVE(ADDRESS_CONSTANT))
	case 252: // r: LONG_LOAD(r,INT_CONSTANT)
	case 248: // rz: INT_2ADDRZerExt(r)
	case 247: // rz: INT_2ADDRZerExt(rz)
	case 244: // stm: LONG_IFCMP(r,LONG_CONSTANT)
	case 243: // stm: LONG_IFCMP(r,INT_CONSTANT)
	case 238: // r: LONG_BITS_AS_DOUBLE(r)
	case 237: // r: DOUBLE_AS_LONG_BITS(r)
	case 236: // r: DOUBLE_2LONG(r)
	case 235: // r: FLOAT_2LONG(r)
	case 234: // r: LONG_2INT(r)
	case 233: // rs: INT_2LONG(rs)
	case 232: // rs: INT_2LONG(r)
	case 230: // r: LONG_USHR(r,INT_CONSTANT)
	case 228: // r: LONG_SHR(r,INT_CONSTANT)
	case 224: // r: LONG_SHL(r,INT_CONSTANT)
	case 223: // r: LONG_REM(r,REF_MOVE(INT_CONSTANT))
	case 221: // r: LONG_DIV(r,REF_MOVE(INT_CONSTANT))
	case 218: // r: LONG_MUL(r,INT_CONSTANT)
	case 206: // stm: RETURN(r)
	case 197: // stm: INT_IFCMP2(r,INT_CONSTANT)
	...
	case 192: // stm: INT_IFCMP(boolcmp,INT_CONSTANT)
	case 182: // stm: INT_IFCMP(r,INT_CONSTANT)
	case 157: // rs: INT_LOAD(r,REF_MOVE(ADDRESS_CONSTANT))
	case 156: // rs: INT_LOAD(r,INT_CONSTANT)
	case 154: // r: DOUBLE_LOAD(r,REF_MOVE(ADDRESS_CONSTANT))
	case 153: // r: DOUBLE_LOAD(r,INT_CONSTANT)
	case 151: // r: FLOAT_LOAD(r,REF_MOVE(ADDRESS_CONSTANT))
	case 150: // r: FLOAT_LOAD(r,INT_CONSTANT)
	case 148: // rp: USHORT_LOAD(r,INT_CONSTANT)
	case 146: // rs: SHORT_LOAD(r,INT_CONSTANT)
	case 144: // rp: UBYTE_LOAD(r,INT_CONSTANT)
	case 140: // rs: BYTE_LOAD(r,INT_CONSTANT)
	case 139: // r: DOUBLE_MOVE(r)
	case 138: // r: FLOAT_MOVE(r)
	case 134: // r: REF_MOVE(r)
	case 133: // r: INT_BITS_AS_FLOAT(r)
	case 132: // r: FLOAT_AS_INT_BITS(r)
	case 131: // r: DOUBLE_2FLOAT(r)
	case 130: // r: DOUBLE_2INT(r)
	case 129: // r: FLOAT_2DOUBLE(r)
	case 128: // r: FLOAT_2INT(r)
	case 127: // r: INT_2DOUBLE(r)
	case 126: // r: INT_2FLOAT(r)
	case 125: // rs: INT_2SHORT(r)
	case 124: // rp: INT_2USHORT(r)
	case 123: // rs: INT_2BYTE(r)
	case 110: // r: DOUBLE_SQRT(r)
	case 109: // r: FLOAT_SQRT(r)
	case 108: // r: DOUBLE_NEG(r)
	case 107: // r: FLOAT_NEG(r)
	case 95: // r: REF_NOT(r)
	case 94: // r: REF_XOR(r,INT_CONSTANT)
	case 90: // r: REF_OR(r,INT_CONSTANT)
	case 84: // rp: REF_AND(r,INT_CONSTANT)
	case 83: // czr: REF_AND(r,INT_CONSTANT)
	case 77: // rp: INT_USHR(r,INT_CONSTANT)
	case 74: // rs: INT_SHR(r,INT_CONSTANT)
	case 71: // rz: INT_SHL(r,INT_CONSTANT)
	case 70: // r: REF_NEG(r)
	case 69: // r: INT_REM(r,REF_MOVE(INT_CONSTANT))
	case 67: // r: INT_DIV(r,REF_MOVE(INT_CONSTANT))
	case 64: // r: INT_MUL(r,INT_CONSTANT)
	case 61: // r: REF_ADD(r,REF_MOVE(INT_CONSTANT))
	case 60: // r: REF_ADD(r,REF_MOVE(INT_CONSTANT))
	case 58: // r: REF_ADD(r,INT_CONSTANT)
	...
	case 54: // r: BOOLEAN_CMP_INT(boolcmp,INT_CONSTANT)
	...
	case 50: // boolcmp: BOOLEAN_CMP_INT(boolcmp,INT_CONSTANT)
	case 48: // boolcmp: BOOLEAN_CMP_ADDR(r,INT_CONSTANT)
	case 46: // r: BOOLEAN_CMP_ADDR(r,INT_CONSTANT)
	case 44: // boolcmp: BOOLEAN_CMP_INT(r,INT_CONSTANT)
	case 42: // r: BOOLEAN_CMP_INT(r,INT_CONSTANT)
	case 41: // r: BOOLEAN_NOT(r)
	case 40: // stm: TRAP_IF(r,LONG_CONSTANT)
	case 39: // stm: TRAP_IF(r,INT_CONSTANT)
	case 36: // stm: ICBI(r)
	case 35: // stm: DCBZL(r)
	case 34: // stm: DCBZ(r)
	case 33: // stm: DCBTST(r)
	case 32: // stm: DCBT(r)
	case 31: // stm: DCBST(r)
	case 30: // stm: DCBF(r)
	...
	case 21: // stm: LOWTABLESWITCH(r)
		if (kidnumber == 0)  return p.child1;
		break;
	case 208: // r: CALL(BRANCH_TARGET,any)
	case 63: // r: REF_SUB(INT_CONSTANT,r)
		if (kidnumber == 0)  return p.child2;
		break;
	case 256: // r: LONG_LOAD(REF_ADD(r,INT_CONSTANT),INT_CONSTANT)
	case 249: // rz: INT_2ADDRZerExt(INT_LOAD(r,INT_CONSTANT))
	case 227: // r: LONG_USHR(LONG_SHL(r,INT_CONSTANT),INT_CONSTANT)
	case 226: // r: LONG_SHL(LONG_USHR(r,INT_CONSTANT),INT_CONSTANT)
	case 191: // stm: INT_IFCMP(REF_AND(r,INT_CONSTANT),INT_CONSTANT)
	case 190: // stm: INT_IFCMP(INT_SHR(r,INT_CONSTANT),INT_CONSTANT)
	case 189: // stm: INT_IFCMP(INT_SHL(r,INT_CONSTANT),INT_CONSTANT)
	case 188: // stm: INT_IFCMP(INT_USHR(r,INT_CONSTANT),INT_CONSTANT)
	case 184: // stm: INT_IFCMP(INT_2SHORT(r),INT_CONSTANT)
	case 183: // stm: INT_IFCMP(INT_2BYTE(r),INT_CONSTANT)
	case 160: // rs: INT_LOAD(REF_ADD(r,INT_CONSTANT),INT_CONSTANT)
	case 143: // rp: REF_AND(BYTE_LOAD(r,INT_CONSTANT),INT_CONSTANT)
	case 88: // rp: REF_AND(INT_USHR(r,INT_CONSTANT),REF_MOVE(INT_CONSTANT))
	case 87: // rp: REF_AND(INT_USHR(r,INT_CONSTANT),INT_CONSTANT)
	case 81: // rp: INT_USHR(INT_SHL(r,INT_CONSTANT),INT_CONSTANT)
	case 80: // rp: INT_USHR(REF_AND(r,REF_MOVE(INT_CONSTANT)),INT_CONSTANT)
	case 79: // rp: INT_USHR(REF_AND(r,INT_CONSTANT),INT_CONSTANT)
	case 76: // rp: INT_SHR(REF_AND(r,INT_CONSTANT),INT_CONSTANT)
	case 73: // rz: INT_SHL(INT_USHR(r,INT_CONSTANT),INT_CONSTANT)
		if (kidnumber == 0)  return p.child1.child1;
		break;
	case 169: // stm: SHORT_STORE(INT_2USHORT(r),OTHER_OPERAND(r,INT_CONSTANT))
	case 167: // stm: SHORT_STORE(INT_2SHORT(r),OTHER_OPERAND(r,INT_CONSTANT))
	case 163: // stm: BYTE_STORE(INT_2BYTE(r),OTHER_OPERAND(r,INT_CONSTANT))
	case 91: // r: REF_OR(REF_NOT(r),REF_NOT(r))
	case 85: // r: REF_AND(REF_NOT(r),REF_NOT(r))
		if (kidnumber == 0)  return p.child1.child1;
		if (kidnumber == 1)  return p.child2.child1;
		break;
	case 258: // stm: LONG_STORE(r,OTHER_OPERAND(r,REF_MOVE(ADDRESS_CONSTANT)))
	case 257: // stm: LONG_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 179: // stm: DOUBLE_STORE(r,OTHER_OPERAND(r,REF_MOVE(ADDRESS_CONSTANT)))
	case 178: // stm: DOUBLE_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 176: // stm: FLOAT_STORE(r,OTHER_OPERAND(r,REF_MOVE(ADDRESS_CONSTANT)))
	case 175: // stm: FLOAT_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 172: // stm: INT_STORE(r,OTHER_OPERAND(r,REF_MOVE(ADDRESS_CONSTANT)))
	case 171: // stm: INT_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 165: // stm: SHORT_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 161: // stm: BYTE_STORE(r,OTHER_OPERAND(r,INT_CONSTANT))
	case 92: // r: REF_OR(r,REF_NOT(r))
	case 86: // r: REF_AND(r,REF_NOT(r))
		if (kidnumber == 0)  return p.child1;
		if (kidnumber == 1)  return p.child2.child1;
		break;
	case 255: // r: LONG_LOAD(REF_ADD(r,r),INT_CONSTANT)
	case 250: // rz: INT_2ADDRZerExt(INT_LOAD(r,r))
	case 246: // stm: INT_IFCMP(ATTEMPT_ADDR(r,r),INT_CONSTANT)
	case 245: // stm: INT_IFCMP(ATTEMPT_INT(r,r),INT_CONSTANT)
	case 187: // stm: INT_IFCMP(INT_SHR(r,r),INT_CONSTANT)
	case 186: // stm: INT_IFCMP(INT_SHL(r,r),INT_CONSTANT)
	case 185: // stm: INT_IFCMP(INT_USHR(r,r),INT_CONSTANT)
	case 159: // rs: INT_LOAD(REF_ADD(r,r),INT_CONSTANT)
	case 142: // rp: REF_AND(BYTE_LOAD(r,r),INT_CONSTANT)
	case 98: // r: REF_NOT(REF_XOR(r,r))
	case 97: // r: REF_NOT(REF_AND(r,r))
	case 96: // r: REF_NOT(REF_OR(r,r))
		if (kidnumber == 0)  return p.child1.child1;
		if (kidnumber == 1)  return p.child1.child2;
		break;
	case 116: // r: DOUBLE_SUB(DOUBLE_MUL(r,r),r)
	case 115: // r: FLOAT_SUB(FLOAT_MUL(r,r),r)
	case 112: // r: DOUBLE_ADD(DOUBLE_MUL(r,r),r)
	case 111: // r: FLOAT_ADD(FLOAT_MUL(r,r),r)
		if (kidnumber == 0)  return p.child1.child1;
		if (kidnumber == 1)  return p.child1.child2;
		if (kidnumber == 2)  return p.child2;
		break;
	case 259: // stm: LONG_STORE(r,OTHER_OPERAND(r,r))
	case 180: // stm: DOUBLE_STORE(r,OTHER_OPERAND(r,r))
	case 177: // stm: FLOAT_STORE(r,OTHER_OPERAND(r,r))
	case 173: // stm: INT_STORE(r,OTHER_OPERAND(r,r))
	case 166: // stm: SHORT_STORE(r,OTHER_OPERAND(r,r))
	case 162: // stm: BYTE_STORE(r,OTHER_OPERAND(r,r))
	case 114: // r: DOUBLE_ADD(r,DOUBLE_MUL(r,r))
	case 113: // r: FLOAT_ADD(r,FLOAT_MUL(r,r))
		if (kidnumber == 0)  return p.child1;
		if (kidnumber == 1)  return p.child2.child1;
		if (kidnumber == 2)  return p.child2.child2;
		break;
	case 122: // r: DOUBLE_NEG(DOUBLE_SUB(DOUBLE_MUL(r,r),r))
	case 121: // r: FLOAT_NEG(FLOAT_SUB(FLOAT_MUL(r,r),r))
	case 118: // r: DOUBLE_NEG(DOUBLE_ADD(DOUBLE_MUL(r,r),r))
	case 117: // r: FLOAT_NEG(FLOAT_ADD(FLOAT_MUL(r,r),r))
		...
		break;
	case 120: // r: DOUBLE_NEG(DOUBLE_ADD(r,DOUBLE_MUL(r,r)))
	case 119: // r: FLOAT_NEG(FLOAT_ADD(r,FLOAT_MUL(r,r)))
		if (kidnumber == 0)  return p.child1.child1;
		if (kidnumber == 1)  return p.child1.child2.child1;
		if (kidnumber == 2)  return p.child1.child2.child2;
		break;
	case 170: // stm: SHORT_STORE(INT_2USHORT(r),OTHER_OPERAND(r,r))
	case 168: // stm: SHORT_STORE(INT_2SHORT(r),OTHER_OPERAND(r,r))
	case 164: // stm: BYTE_STORE(INT_2BYTE(r),OTHER_OPERAND(r,r))
		if (kidnumber == 0)  return p.child1.child1;
		if (kidnumber == 1)  return p.child2.child1;
		if (kidnumber == 2)  return p.child2.child2;
		break;
	case 260: // stm: LONG_STORE(r,OTHER_OPERAND(REF_ADD(r,INT_CONSTANT),INT_CONSTANT))
	case 174: // stm: INT_STORE(r,OTHER_OPERAND(REF_ADD(r,INT_CONSTANT),INT_CONSTANT))
		if (kidnumber == 0)  return p.child1;
		if (kidnumber == 1)  return p.child2.child1.child1;
		break;
	case 251: // rz: INT_2ADDRZerExt(INT_LOAD(REF_ADD(r,INT_CONSTANT),INT_CONSTANT))
		if (kidnumber == 0)  return p.child1.child1.child1;
		break;
	}
...
void code219(BURS_TreeNode p) {
    EMIT(MIR_Binary.mutate(P(p), PPC64_MULLD, Binary.getResult(P(p)),                                      R(Binary.getVal1(P(p))), Binary.getVal2(P(p))));
}
void code220(BURS_TreeNode p) {
    EMIT(MIR_Binary.mutate(P(p), PPC64_DIVD, GuardedBinary.getResult(P(p)),                         R(GuardedBinary.getVal1(P(p))), GuardedBinary.getVal2(P(p))));
}
void code221(BURS_TreeNode p) {
    LONG_DIV_IMM(P(p), GuardedBinary.getResult(P(p)), R(GuardedBinary.getVal1(P(p))),                    Move.getResult(PR(p)), IC(Move.getVal(PR(p))));
}
...
void code223(BURS_TreeNode p) {
    LONG_REM_IMM(P(p), GuardedBinary.getResult(P(p)), R(GuardedBinary.getVal1(P(p))),                    Move.getResult(PR(p)), IC(Move.getVal(PR(p))));
}
void code224(BURS_TreeNode p) {
    EMIT(MIR_Binary.mutate(P(p), PPC64_SLDI, Binary.getResult(P(p)),                                     R(Binary.getVal1(P(p))), Binary.getVal2(P(p))));
}
void code225(BURS_TreeNode p) {
    EMIT(MIR_Binary.mutate(P(p), PPC64_SLD, Binary.getResult(P(p)),                                      R(Binary.getVal1(P(p))), Binary.getVal2(P(p))));
}
void code226(BURS_TreeNode p) {
    EMIT(MIR_RotateAndMask.mutate(P(p), PPC64_RLDICR, Binary.getResult(P(p)), R(Binary.getVal1(PL(p))),                               IC(VR(p) - VLR(p)), IC(0), IC(63 - VR(p))));
}
void code227(BURS_TreeNode p) {
    EMIT(MIR_RotateAndMask.mutate(P(p), PPC64_RLDICL, Binary.getResult(P(p)), R(Binary.getVal1(PL(p))),                               IC(64 - (VR(p) - VLR(p))), IC(VR(p)), IC(63)));
}
...
void code231(BURS_TreeNode p) {
    EMIT(MIR_Binary.mutate(P(p), PPC64_SRD, Binary.getResult(P(p)),                                      R(Binary.getVal1(P(p))), Binary.getVal2(P(p))));
}
...
void code246(BURS_TreeNode p) {
    {                                                                                ConditionOperand c = IfCmp.getCond(P(p)).flipCode();                      EMIT(MIR_Store.create(PPC_STAddrCXr, R(Attempt.getNewValue(PL(p))),                                   R(Attempt.getAddress(PL(p))), Attempt.getOffset(PL(p)),                                             Attempt.getLocation(PL(p)),                                                  Attempt.getGuard(PL(p))));                   EMIT(MIR_CondBranch.mutate(P(p), PPC_BCOND, CR(0),                                                       new PowerPCConditionOperand(c),                                           IfCmp.getTarget(P(p)),                                                       IfCmp.getBranchProfile(P(p))));                 }
}
...
void code249(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(PL(p), PPC_LWZ, Unary.getResult(P(p)),                                 R(Load.getAddress(PL(p))), Load.getOffset(PL(p)),                      Load.getLocation(PL(p)), Load.getGuard(PL(p))));
}
void code250(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(PL(p), PPC_LWZX, Unary.getResult(P(p)),                                R(Load.getAddress(PL(p))), Load.getOffset(PL(p)),                      Load.getLocation(PL(p)), Load.getGuard(PL(p))));
}
void code251(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(PL(p), PPC_LWZ, Unary.getResult(P(p)),                                          R(Binary.getVal1(PLL(p))), IC(VLR(p)+VLLR(p)),                               Load.getLocation(PL(p)), Load.getGuard(PL(p))));
}
void code252(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(P(p), PPC_LAddr, Load.getResult(P(p)),                                 R(Load.getAddress(P(p))), Load.getOffset(P(p)), 	             Load.getLocation(P(p)), Load.getGuard(P(p))));
}
void code253(BURS_TreeNode p) {
    {                                                                                Address val = AV(Move.getVal(PR(p)));                                         EMIT(MIR_Binary.create(PPC_ADDIS, Move.getResult(PR(p)),                                             R(Load.getAddress(P(p))), CAU16(val)));                EMIT(MIR_Load.mutate(P(p), PPC_LAddr, Load.getResult(P(p)),                                          Move.getResult(PR(p)).copyRO(), CAL16(val),  				 Load.getLocation(P(p)),                                                       Load.getGuard(P(p))));                      }
}
void code254(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(P(p), PPC_LAddrX, Load.getResult(P(p)),                                R(Load.getAddress(P(p))), Load.getOffset(P(p)), 	             Load.getLocation(P(p)), Load.getGuard(P(p))));
}
void code255(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(P(p), PPC_LAddrX, Load.getResult(P(p)),                                         R(Binary.getVal1(PL(p))), Binary.getVal2(PL(p)), 			      Load.getLocation(P(p)), Load.getGuard(P(p))));
}
void code256(BURS_TreeNode p) {
    EMIT(MIR_Load.mutate(P(p), PPC_LAddr, Load.getResult(P(p)),                                          R(Binary.getVal1(PL(p))), IC(VR(p)+VLR(p)), 			      Load.getLocation(P(p)), Load.getGuard(P(p))));
}
void code257(BURS_TreeNode p) {
    EMIT(MIR_Store.mutate(P(p), PPC_STAddr, R(Store.getValue(P(p))),                                       R(Store.getAddress(P(p))),                                                    Store.getOffset(P(p)),                         			       Store.getLocation(P(p)),                                                      Store.getGuard(P(p))));
}
void code258(BURS_TreeNode p) {
    {                                                                                Address val = AV(Move.getVal(PRR(p)));                                        EMIT(MIR_Binary.create(PPC_ADDIS, Move.getResult(PRR(p)),                                            R(Store.getAddress(P(p))), CAU16(val)));               EMIT(MIR_Store.mutate(P(p), PPC_STAddr, R(Store.getValue(P(p))),                                       Move.getResult(PRR(p)).copyRO(), CAL16(val),				  Store.getLocation(P(p)),                                                      Store.getGuard(P(p))));                    }
}
void code259(BURS_TreeNode p) {
    EMIT(MIR_Store.mutate(P(p), PPC_STAddrX, R(Store.getValue(P(p))),                                      R(Store.getAddress(P(p))),                                                    Store.getOffset(P(p)),                         			       Store.getLocation(P(p)),                                                      Store.getGuard(P(p))));
}
void code260(BURS_TreeNode p) {
    EMIT(MIR_Store.mutate(P(p), PPC_STAddr, R(Store.getValue(P(p))),                                       R(Binary.getVal1(PRL(p))),                                                    IC(VRR(p) + VRLR(p)), 			       Store.getLocation(P(p)),                                                      Store.getGuard(P(p))));
}
